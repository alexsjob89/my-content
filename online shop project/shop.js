...
const currencyLists = document.querySelector(".currencyLists");
...
currencyLists.addEventListener("mouseleave", () => {
    currencys.style.display = "none"
})
...
// here starts bottom slidebar function

let loop = horizontalLoop(".image", { speed: 0.5, repeat: -1, paddingRight: 60 });

function setDirection(value) {
    if (loop.direction !== value) {
        gsap.to(loop, { timeScale: value, duration: 5, overwrite: true });
...
function horizontalLoop(items, config) {
    ...
    let tl = gsap.timeline({ repeat: config.repeat, paused: config.paused, defaults: { ease: "none" }, onReverseComplete: () => tl.totalTime(tl.rawTime() + tl.duration() * 100) }),
        ...
        xPercents = [],
        curIndex = 0,
        pixelsPerSecond = (config.speed || 4) * 100,
